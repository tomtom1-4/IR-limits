...
int main() {
  ...
  XMLDocument xmlDoc;
  XMLError eResult = xmlDoc.LoadFile(const_cast<char*>(("results/ColorCorrelators/" + process_str + ".xml").c_str()));
  XMLNode * pRoot = xmlDoc.FirstChild();
  if(pRoot == nullptr) {
    std::cout << "Could not find file: " << "results/ColorCorrelators/" + process_str + ".xml" << std::endl;
    return -1;
  }
  ...
  std::unordered_map<std::string, double> M0_ij, M1_ij, fM0_ijk, fM1_ijk, dM_ijk, M0_ijkl, M1_ijkl, M0_ijklab, Q_ijkl, M0_ijkla; // color correlators
  ...
  while (scale > 1.e-6) {
    ...
    //double M2_quad = soft_gg_squared_1l_quad(pp_full, M0_ij, M1_ij, M0_ijkl, M1_ijkl, M0_ijkla, Q_ijkl, A, n_f)*average_factor_full/average_factor;
