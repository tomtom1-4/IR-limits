  ...
  while (topologies.next_topology())
    {
      ...
      if (topologies.current_topology().has_loops()) continue;

      DiagramGenerator diags(topologies.current_topology(),QCD,
                           {"A"},{"p123"},{"q","qbar","g"},{"p1","p2","p3"});
      ...
      while (diags.next_diagram()){
        ...
        if (!internal_photon) {
          if(new_top) {
            topologies.current_topology().postscript_print("topologies/top" + std::to_string(counter));
            new_top = false;
          }
          print_diagram(diags_out,n_diags,diags.current_diagram());
          counter++;
        }
      }
    }

  diags_out << "#define ndiags \"" << n_diags << "\"\n";
