...
int main()
{
  ...
  // tree-level diagrams with 3 gluons

  TopologyGenerator topologies_3g(6,0);
  topologies_3g.enable(TopologyGenerator::NoTadpoles);

  int n_diags = 0;

  ofstream diags_ggg("diags_ggg.out");

  while (topologies_3g.next_topology())
    {
      // there are only massless fields in the Lagrangian
      // hence any (graph-theory) loop will yield a scaless integral
      if (topologies_3g.current_topology().has_loops()) continue;

      DiagramGenerator diags(topologies_3g.current_topology(),QCD,
                           {"A"},{"p12345"},{"q","qbar","g","g","g"},{"p1","p2","p3","p4","p5"});
      ...
      while (diags.next_diagram()){
        ...
        if (!internal_photon) {
          print_diagram(diags_ggg,n_diags,diags.current_diagram());
        }
      }
    }

  diags_ggg << "#define ndiags \"" << n_diags << "\"\n";


  diags_ggg.close();

  // tree-level diagrams with 1 gluon and 2 soft quarks

  TopologyGenerator topologies_1g(6,0);
  topologies_1g.enable(TopologyGenerator::NoTadpoles);
  topologies_1g.enable(TopologyGenerator::OnShell);

  n_diags = 0;

  ofstream diags_qqg("diags_qqg.out");
  int counter = 0;

  while (topologies_1g.next_topology())
    {
      // there are only massless fields in the Lagrangian
      // hence any (graph-theory) loop will yield a scaless integral
      if (topologies_1g.current_topology().has_loops()) continue;

      DiagramGenerator diags(topologies_1g.current_topology(),QCD,
                           {"A"},{"p12345"},{"q","qbar","f","fbar","g"},{"p1","p2","p3","p4","p5"});
      //topologies_1g.current_topology().postscript_print("topologies/top" + std::to_string(counter));
      while (diags.next_diagram()){
        bool internal_photon = false;
        for (int e = 0; e < diags.current_diagram().n_edges(); ++e)
            if ((!diags.current_diagram().is_external_edge(e)) && (diags.current_diagram().line(e).first->_name == "A")) {
        ...
        if (!internal_photon) {
          print_diagram(diags_qqg,n_diags,diags.current_diagram());
          counter++;
        }
      }
    }

  diags_qqg << "#define ndiags \"" << n_diags << "\"\n";
